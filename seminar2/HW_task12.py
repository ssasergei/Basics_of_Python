...
# а Катя – школьница. Петя помогает Кате по математике. Он задумывает два натуральных числа X и Y (X,Y≤1000),
...
x = int(input('Придумайте за Петю число X: '))
y = int(input('Придумайте за Петю число Y: '))
for i in range(x):
    for j in range(y):
        if x == i + j and y == i * j:
            print(x, y, '->', i, j)